...
namespace :data_comparison do
  ...
  task :defendant_data, [:case_list] => [:environment] do |_task, _args|
    include Api::Internal::V1
    puts _args
    case_urns = _args[:case_list].split("-")
...
def get_v1_defendant_json(defendant_id)
  ...
  JSON.parse(DefendantSerializer.new(defendant, include: ['offences']).to_json)
end
