RSpec.describe MaatApi::CourtApplication, type: :model do
  let(:hearing_body) { JSON.parse(file_fixture("hearing_resulted/with_court_application.json").read).deep_symbolize_keys }
  ...
  context "when the hearing body has only required fields" do
    let(:hearing_body) { JSON.parse(file_fixture("hearing_resulted/with_court_application_required_only.json").read).deep_symbolize_keys }
