...
RSpec.describe Api::GetHearingResults do
  context "when getting result by hearing id only" do
    ...
    before do
      allow(HearingFetcher).to receive(:call).with(hearing_id: hearing_id, sitting_day: nil).and_return(response)
    end
    ...
    context "when publish_to_queue is enabled" do
      subject(:get_hearing_results) { described_class.call(hearing_id: hearing_id, sitting_day: nil, publish_to_queue: true) }
  ...
  context "when getting results by hearing id and hearing date" do
    ...
    before do
      allow(HearingFetcher).to receive(:call).with(hearing_id: hearing_id, sitting_day: sitting_day).and_return(response)
    end
